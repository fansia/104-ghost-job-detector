...
| 螢幕截圖 | 1280×800 或 640×400,1–5 張 | ❌ **現有的已過期,必須重拍** |
| 小型宣傳圖塊 | 440×280 PNG(選填,但會影響曝光) | ✅ `store/promo-440x280.png` |
| 跑馬燈宣傳圖塊 | 1400×560 PNG(選填) | ✅ `store/promo-1400x560.png` |
| 開發者帳號 | 一次性 $5 USD 註冊費 | ✅ 已註冊,v1.0.2 已送審 |

### ⚠️ 現有截圖不能用

`store/screenshot-1280x800.png` 是 v1.0.2 時期拍的,裡面的徽章寫著
**「從未回覆應徵者」** —— 那正是 v1.1.0 更正掉的錯誤說法。送上去會跟新的描述
自相矛盾,等於把已知錯誤的內容公開展示。**必須重拍。**

同時它也沒有 v1.2.0 新增的「機會」徽章。

### 要重拍哪些

用 v1.3.1 或更新的版本,在實際的 104 頁面上拍:

1. **搜尋結果頁**,多張卡片都掛著徽章 —— 最好同時看得到風險徽章與綠色的「✦ 機會」徽章
2. **「✦ 機會」徽章**,例如
   `✦ 機會 目前只有 2 人應徵 ・ 今天刊登 ・ HR 1 天內處理過履歷`
3. **徽章展開後的完整數據表**,可對照 104 自己只顯示「6~10 人應徵」而徽章給精確數字

拍好交給我,我會排版成合規的 1280×800、24 位元 RGB PNG(無 alpha 透明層)。
macOS 截圖預設帶 alpha,直接上傳會被商店擋下來。
